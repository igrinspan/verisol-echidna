...
contract RefrigeratedTransportation {
	...
	function vc1x0x0(int humidity, int temperature, int timestamp, address newCounterparty) hasInitialized payable public {
		require((State != StateType.Completed && State != StateType.OutOfCompliance)  && !((State != StateType.Completed && State != StateType.OutOfCompliance)) && !((State != StateType.Completed && State != StateType.OutOfCompliance)));
		require(Device == msg.sender);
		IngestTelemetry(humidity, temperature, timestamp);
		assert(!(!((State != StateType.Completed && State != StateType.OutOfCompliance) ) && !((State != StateType.Completed && State != StateType.OutOfCompliance)) && !((State != StateType.Completed && State != StateType.OutOfCompliance))&& true));
	}
