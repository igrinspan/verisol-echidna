...
contract Crowdfunding {
	...
	function vc0x0x0() hasInitialized payable public {
		require((!(max_block > blockNumber) && !(max_block < blockNumber && goal <= balance) && !(blockNumber > max_block && !funded && goal > balance && backersArray.length != 0) && balance == 0 && max_block < blockNumber));
		Donate();
		assert(!((!(max_block > blockNumber) && !(max_block < blockNumber && goal <= balance) && !(blockNumber > max_block && !funded && goal > balance && backersArray.length != 0) && balance == 0 && max_block < blockNumber)&& true));
	}
