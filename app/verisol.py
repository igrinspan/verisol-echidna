...
def output_transitions_function(
    preconditionRequire, function, preconditionAssert, functionIndex, extraConditionPre, extraConditionPost
):
    if mode == Mode.epa:
    ...
    verisolFucntionOutput = (
        ...
        + preconditionRequire
        + ");\nrequire("
        + precondictionFunction
        + ");\n"
        + extraConditionOutputPre
        + function
        + "\n"
        + "assert(!("
        + preconditionAssert
...
def print_output(
    indexPreconditionRequire, indexFunction, indexPreconditionAssert, combinations, fullCombination, succes_by_to
):
    output = (
        "Desde este estado:\n"
        + output_combination(indexPreconditionRequire, combinations)
        + "\nHaciendo "
        + str(functions[indexFunction] + succes_by_to)
        + "\n\nLlegas al estado:\n"
...
def get_valid_transitions_output(
    preconditionsThread, preconditions, extraConditionsTemp, extraConditions, functions, statesThread
):
    global mode
    temp_output = ""
    tempFunctionNames = []
        ...
        for indexPreconditionAssert, preconditionAssert in enumerate(preconditions):
            for indexFunction, function in enumerate(functions):
                ...
                if (indexFunction + 1) in statesThread[indexPreconditionRequire] and mode == Mode.epa:
                    functionName = get_temp_function_name(
                        indexPreconditionRequire, indexPreconditionAssert, indexFunction
                    )
                    tempFunctionNames.append(functionName)
                    temp_function = functionOutput(functionName) + "\n"
...
def try_preconditions(tool, tempFunctionNames, final_directory, statesTemp, preconditionsTemp, extraConditionsTemp, arg):
    ...
    for functionName in tempFunctionNames:
        ...
        query_result = try_command(
            tool, functionName, tempFunctionNames, final_directory, statesTemp, txBound, time_out, False
        )
        if query_result[1] == TRACK_VARS:
            query_result = try_command(
                tool, functionName, tempFunctionNames, final_directory, statesTemp, txBound, time_out, True
            )
...
def try_transaction(tool, tempFunctionNames, final_directory, statesTemp, states, arg):
    ...
    for functionName in tempFunctionNames:
        ...
        if success:
            add_node_to_graph(
                indexPreconditionRequire, indexPreconditionAssert, indexFunction, statesTemp, states, succes_by_timeout
            )
            print_output(
                indexPreconditionRequire, indexFunction, indexPreconditionAssert, statesTemp, states, succes_by_timeout
            )
...
def try_init(tool, tempFunctionNames, final_directory, states):
    ...
    for functionName in tempFunctionNames:
        ...
        query_result = try_command(
            tool, functionName, tempFunctionNames, final_directory, [], txBound_constructor, time_out, False
        )
        if query_result[1] == TRACK_VARS:
            query_result = try_command(
                tool, functionName, tempFunctionNames, final_directory, [], txBound_constructor, time_out, True
            )
        success = query_result[0]
        ...
        if success:
            dot.node("init", "init")
            dot.node(
                combinationToString(states[indexPreconditionAssert]),
...
def try_command(
    tool, temp_function_name, tempFunctionNames, final_directory, statesTemp, txBound, time_out, trackAllVars
):
    ...
    try:
        ...
        indexPreconditionRequire, indexPreconditionAssert, indexFunction = get_params_from_function_name(
            temp_function_name
        )
    ...
    output_verisol = result.stdout.decode()
...
def add_node_to_graph(
    indexPreconditionRequire, indexPreconditionAssert, indexFunction, statesTemp, states, succes_by_to
):
    global dot, functions
    dot.node(
        combinationToString(statesTemp[indexPreconditionRequire]),
    ...
    dot.edge(
        combinationToString(statesTemp[indexPreconditionRequire]),
        combinationToString(states[indexPreconditionAssert]),
        label=str(functions[indexFunction] + succes_by_to),
    )
...
def reduceCombinations(arg):
    ...
    preconditionsTemp2, statesTemp2, extraConditionsTemp2 = try_preconditions(
        tool, fuctionCombinations, final_directory, statesTemp, preconditionsTemp, extraConditionsTemp, arg
    )
...
def validCombinations(arg):
    ...
    final_directory = create_directory(arg)
    fileNameTemp = create_file(arg, final_directory)
    body, fuctionCombinations = get_valid_transitions_output(
        preconditionsTemp, preconditions, extraConditionsTemp, extraConditions, functions, statesTemp
    )
    write_file(fileNameTemp, body)
    tool = "VeriSol " + fileNameTemp + " " + contractName
...
def main():
    ...
    if True:  # Esto es prepare_variables.
        if mode == Mode.epa:
    ...
    if True:  # Definimos la cantidad de threads y dividimos las cosas.
        threadCount = 8
        threads = []
    ...
    if True:  # Se ejecuta el reduceCombinations (sólo modo epa)
    ...
    # Actualizamos los valores de preconditions, states y extraConditions,
    if True:  # basándonos en los resultados de reduceCombinations
    ...
    countPreFinal = len(preconditions)  # Esto es para ver cuántas preconditions descartamos.

    if True:  # Guardamos los resultados de reduceCombinations en un archivo
    ...
    # Si hay menos preconditions que threadCount, reducimos threadCount.
    realThreadCount = threadCount if len(preconditionsThreads) > threadCount else len(preconditionsThreads)

    if True:  # Volvemos a dividir las cosas, para la nueva cantidad de threads
        threads = []
        divideThreads = 1
        moduleThreadsCount = 0
        ...
        if len(preconditionsThreads) > 30:
            ...
            moduleThreadsCount = divideCount % threadCount
    ...
    if True:  # Se ejecuta el validCombinations (hay 2 maneras posibles de hacerlo)
    ...
    if True:  # ¿Se ejecuta validCombinations de nuevo?
        threads = []
        for index in range(moduleThreadsCount):
            thread = Thread(target=validCombinations, args=[threadCount * divideThreads + index])
            thread.start()
            threads.append(thread)

    if True:  # Se ejecuta validInit
