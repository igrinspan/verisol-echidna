...
contract RefundEscrow is ConditionalEscrow {
	...
	function vc0x0x4(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(!(_state == State.Active) && !(_state == State.Active) && !(_state == State.Active) && !(_state == State.Closed) && true && true);
		withdraw(payee);
		assert(!(!(_state == State.Active) && !(_state == State.Active) && !(_state == State.Active) && !(_state == State.Closed) && true && true&& true));
	}
	...
	function vc0x3x4(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(!(_state == State.Active) && !(_state == State.Active) && !(_state == State.Active) && !(_state == State.Closed) && true && true);
		withdraw(payee);
		assert(!(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true&& true));
	}
	function vc0x3x5(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(!(_state == State.Active) && !(_state == State.Active) && !(_state == State.Active) && !(_state == State.Closed) && true && true);
		require(recipient != address(0));
		transferPrimary(recipient);
		assert(!(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true&& true));
	...
	function vc1x3x3(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(!(_state == State.Active) && !(_state == State.Active) && !(_state == State.Active) && _state == State.Closed && true && true);
		beneficiaryWithdraw();
		assert(!(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true&& true));
	}
	function vc1x3x4(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(!(_state == State.Active) && !(_state == State.Active) && !(_state == State.Active) && _state == State.Closed && true && true);
		withdraw(payee);
		assert(!(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true&& true));
	}
	function vc1x3x5(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(!(_state == State.Active) && !(_state == State.Active) && !(_state == State.Active) && _state == State.Closed && true && true);
		require(recipient != address(0));
		transferPrimary(recipient);
		assert(!(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true&& true));
	...
	function vc2x3x0(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && !(_state == State.Closed) && true && true);
		deposit(refundee);
		assert(!(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true&& true));
	}
	function vc2x3x1(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && !(_state == State.Closed) && true && true);
		close();
		assert(!(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true&& true));
	}
	function vc2x3x2(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && !(_state == State.Closed) && true && true);
		enableRefunds();
		assert(!(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true&& true));
	}
	function vc2x3x4(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && !(_state == State.Closed) && true && true);
		withdraw(payee);
		assert(!(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true&& true));
	}
	function vc2x3x5(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && !(_state == State.Closed) && true && true);
		require(recipient != address(0));
		transferPrimary(recipient);
		assert(!(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true&& true));
	...
	function vc3x0x1(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true);
		close();
		assert(!(!(_state == State.Active) && !(_state == State.Active) && !(_state == State.Active) && !(_state == State.Closed) && true && true&& true));
	}
	function vc3x0x2(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true);
		enableRefunds();
		assert(!(!(_state == State.Active) && !(_state == State.Active) && !(_state == State.Active) && !(_state == State.Closed) && true && true&& true));
	}
	function vc3x0x3(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true);
		beneficiaryWithdraw();
		assert(!(!(_state == State.Active) && !(_state == State.Active) && !(_state == State.Active) && !(_state == State.Closed) && true && true&& true));
	}
	function vc3x0x4(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true);
		withdraw(payee);
		assert(!(!(_state == State.Active) && !(_state == State.Active) && !(_state == State.Active) && !(_state == State.Closed) && true && true&& true));
	}
	function vc3x0x5(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true);
		require(recipient != address(0));
		transferPrimary(recipient);
		assert(!(!(_state == State.Active) && !(_state == State.Active) && !(_state == State.Active) && !(_state == State.Closed) && true && true&& true));
	}
	function vc3x1x0(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true);
		deposit(refundee);
		assert(!(!(_state == State.Active) && !(_state == State.Active) && !(_state == State.Active) && _state == State.Closed && true && true&& true));
	}
	function vc3x1x1(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true);
		close();
		assert(!(!(_state == State.Active) && !(_state == State.Active) && !(_state == State.Active) && _state == State.Closed && true && true&& true));
	}
	function vc3x1x2(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true);
		enableRefunds();
		assert(!(!(_state == State.Active) && !(_state == State.Active) && !(_state == State.Active) && _state == State.Closed && true && true&& true));
	}
	function vc3x1x3(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true);
		beneficiaryWithdraw();
		assert(!(!(_state == State.Active) && !(_state == State.Active) && !(_state == State.Active) && _state == State.Closed && true && true&& true));
	}
	function vc3x1x4(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true);
		withdraw(payee);
		assert(!(!(_state == State.Active) && !(_state == State.Active) && !(_state == State.Active) && _state == State.Closed && true && true&& true));
	}
	function vc3x1x5(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true);
		require(recipient != address(0));
		transferPrimary(recipient);
		assert(!(!(_state == State.Active) && !(_state == State.Active) && !(_state == State.Active) && _state == State.Closed && true && true&& true));
	}
	function vc3x2x0(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true);
		deposit(refundee);
		assert(!(_state == State.Active && _state == State.Active && _state == State.Active && !(_state == State.Closed) && true && true&& true));
	}
	function vc3x2x1(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true);
		close();
		assert(!(_state == State.Active && _state == State.Active && _state == State.Active && !(_state == State.Closed) && true && true&& true));
	}
	function vc3x2x2(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true);
		enableRefunds();
		assert(!(_state == State.Active && _state == State.Active && _state == State.Active && !(_state == State.Closed) && true && true&& true));
	}
	function vc3x2x3(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true);
		beneficiaryWithdraw();
		assert(!(_state == State.Active && _state == State.Active && _state == State.Active && !(_state == State.Closed) && true && true&& true));
	}
	function vc3x2x4(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true);
		withdraw(payee);
		assert(!(_state == State.Active && _state == State.Active && _state == State.Active && !(_state == State.Closed) && true && true&& true));
	}
	function vc3x2x5(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true);
		require(recipient != address(0));
		transferPrimary(recipient);
		assert(!(_state == State.Active && _state == State.Active && _state == State.Active && !(_state == State.Closed) && true && true&& true));
	}
	function vc3x3x0(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true);
		deposit(refundee);
		assert(!(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true&& true));
	}
	function vc3x3x1(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true);
		close();
		assert(!(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true&& true));
	}
	function vc3x3x2(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true);
		enableRefunds();
		assert(!(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true&& true));
	}
	function vc3x3x3(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true);
		beneficiaryWithdraw();
		assert(!(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true&& true));
	}
	function vc3x3x4(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true);
		withdraw(payee);
		assert(!(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true&& true));
	}
	function vc3x3x5(address refundee, address payable payee, address recipient) hasInitialized payable public {
		require(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true);
		require(recipient != address(0));
		transferPrimary(recipient);
		assert(!(_state == State.Active && _state == State.Active && _state == State.Active && _state == State.Closed && true && true&& true));
     ...
     */
    function deposit(address refundee) hasInitialized public payable {
